...
use progset1::{
    ...
    graph_gen::{CompleteUnitGraph, Graph, Vertex0D, Vertex2D, Vertex3D, Vertex4D, VertexCoord},
    mst::Mst,
    Weight,
};
use std::{
...
fn mst_average(
    ...
    for _ in 0..num_trials {
        handles.push(thread::spawn(move || match dimension {
            ...
            GraphDim::FourD => run_trial::<Vertex4D>(num_vertices),
        }));
    }
...
fn run_trial<V>(num_vertices: usize) -> (Weight<f64>, Duration)
where
    V: VertexCoord<V> + Eq + Hash + Debug + Default,
{
    let timer = Instant::now();
    let graph: Graph<V> = CompleteUnitGraph::graph_nd(num_vertices);
